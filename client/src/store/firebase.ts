...
class BrowserFirestoreStore implements BrowserStore {
  ...
  /**
   * WAIT FOR WHOEVER IS SIGNED IN. It does not sign anybody in.
   *
   * ORDER 0064. This used to be `signInAnonymously(this.auth)`, unconditionally, in the
   * constructor -- with a comment arguing that anonymous was sufficient because "the rules only
   * ask whether this uid is a member". The comment was right about the rules and wrong about the
   * product: a throwaway uid is a member of nothing, so a user whose project belongs to their
   * Google account saw an empty board and a correct-looking explanation for it. The two
   * identities could never meet, because the board had no way to become the person.
   *
   * So the store now OBSERVES the session and App owns the choice. Anonymous is still available
   * -- the denied-state onboarding path depends on it, and the edge harness asserts it -- but it
   * is a button, not a default.
   *
   * onAuthStateChanged rather than reading auth.currentUser once: a restored session arrives
   * asynchronously, so a one-shot read is null for the first few milliseconds after every reload.
   */
  private async beginSignIn(): Promise<string> {
    try {
      const uid = await new Promise<string>((resolve, reject) => {
        const stop = watchSession(this.auth, (s) => {
          if (!s) return; // not signed in yet; App is showing the sign-in screen
          stop();
          resolve(s.uid);
        });
        // Not a timeout on the whole wait -- someone may sit on the sign-in screen for a minute.
        // The auth object itself failing is what is caught here, via the promise it exposes.
        this.auth.authStateReady?.().catch(reject);
      });
      ...
      const { code, detail } = explainAuthError(err);
      this.setStatus({ state: 'auth-unavailable', code, detail });
      throw err;
    }
  }
...
// authDomainFor and configFromEnv moved to ./config (order 0064) so that session.ts can use
// them without firebase.ts and session.ts importing each other. Re-exported here because that is
// where every caller and the edge harness already look for them.
import { configFromEnv } from './config';
export { authDomainFor, configFromEnv } from './config';
